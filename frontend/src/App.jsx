...
const App = () => {
  ...
  const handleDownload = async (type) => {
    try {
      const blob =
        type === 'csv' ? await api.downloadCSV() : await api.downloadXLSX();
  ...
  const stats = useMemo(() => {
    ...
    const averageAge = total
      ? Math.round(
          nurses.reduce((sum, nurse) => sum + Number(nurse.age || 0), 0) / total
        )
      : 0;
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500">
                Manage records, exports, and inline tasks efficiently.
              </p>
            </div>
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <SearchBar
              ...
              onRefresh={() => loadNurses({})}
            />
            <div className="flex gap-3 justify-between">
              <button
                onClick={() => handleDownload('csv')}
                className="rounded-lg border w-2xs border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                Download CSV
              </button>
              <button
